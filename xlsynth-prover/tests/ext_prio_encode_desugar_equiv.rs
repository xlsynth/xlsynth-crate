...
#[test]
fn ext_prio_encode_equivalent_to_desugared_export_form_and_exports_to_upstream() {
    for w in 1u64..=16u64 {
        let out_w = xlsynth_pir::math::ceil_log2((w as usize).saturating_add(1));
        for lsb_prio in [false, true] {
            let ir = format!(
                "package test\n\nfn f(arg: bits[{w}] id=1) -> bits[{out_w}] {{\n  ret r: bits[{out_w}] = ext_prio_encode(arg,lsb_prio={lsb_prio},id=2)\n}}\n"
            );
            let pkg = {
            ...
            assert!(
                !exported.contains("ext_prio_encode"),
                "export should not contain ext_prio_encode:\n{}",
                exported
            );
