...
message AigProvenance {
  ...
  repeated IrNode ir_nodes = 6; // Indexed; sorted by text_id.
  ...
  repeated OutputBitRun exact_output_bits = 10;
}
```
...
contiguous AIG IDs; singleton runs remain exact in other cases. Compression of
the entire `.pb` as `.pb.zst` can be added without changing the schema.
...
1. Add an optional `--provenance <PATH>` to `aig-toggle-hotspots`; verify the
   ...
   only where `exact_output_bits` says they are available.
