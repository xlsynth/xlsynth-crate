...
///
/// The bindings are retained so `xlsynth-sys` covers libxls APIs that are not
/// implemented natively in Rust. The high-level xlsynth AOT API uses the
/// PIR/Cranelift compiler.
#[repr(C)]
