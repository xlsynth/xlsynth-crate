...
const RELEASE_LIB_VERSION_TAG: &str = "v0.59.0";
...
fn write_artifact_paths_rs(out_dir: &Path, artifact_paths: &ArtifactPaths) {
    ...
    let file_contents = format!(
        ...
        dslx_stdlib_path = artifact_paths.dslx_stdlib_path,
        dso_path = artifact_paths.dso_path,
    );
...
fn main() {
    ...
    if std::env::var("DEV_XLS_DSO_WORKSPACE").is_ok() {
        ...
        write_artifact_paths_rs(
            &out_dir,
            &ArtifactPaths {
                dso_path: out_dir.display().to_string(),
                dslx_stdlib_path: stdlib_path_full,
            },
        );
        println!("cargo:DSO_PATH={}", out_dir.display());
        return;
    }
    ...
    // DO NOT USE THIS FROM WITHIN YOUR build.rs. See the definition in lib.rs
    // for an explanation.
    write_artifact_paths_rs(
        &out_dir,
        &ArtifactPaths {
