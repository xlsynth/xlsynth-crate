...
impl GateFnFragment {
    ...
    ///
    /// This is intended for taking a canonical recipe (canonical input order) and
    /// producing a recipe over original inputs. See `crate::cut_db::npn` for the
    /// transform semantics.
    ...
    pub fn to_gatefn(&self, name: &str) -> GateFn {
        ...
        let mut ops: Vec<crate::aig::AigOperand> = Vec::with_capacity(FIRST_NODE_ID as usize + self.nodes.len());
