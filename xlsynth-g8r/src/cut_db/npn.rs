...
//! - output negation (N)
//!
//! We compute a canonical representative for a `TruthTable16` by enumerating all
//! 768 transforms and selecting the smallest transformed `u16` value.

use serde::{Deserialize, Serialize};

use crate::cut_db::tt16::{decode_assignment, encode_assignment, TruthTable16};
...
///   assignment `x` (original domain) such that for each i in 0..4:
///   `x[perm[i]] = y[i] XOR input_neg(i)`.
...
/// (and optionally negated) original inputs, and optionally negating the output.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize, Deserialize)]
    ...
    for perm in perms {
        for input_neg_mask in 0u8..16 {
            for &output_neg in &[false, true] {
                ...
                // Note: cand_tt primary ensures stable canonical representative by numeric order.
                if cand.0 < best_tt.0
                    || (cand.0 == best_tt.0
