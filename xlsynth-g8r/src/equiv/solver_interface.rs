...
pub trait Solver: Sized + Clone {
    type Rep: Clone + std::fmt::Debug;
...
pub mod test_utils {
    ...
    pub fn test_bitvec_equiv_basic<S: Solver>(solver: &mut S) {
        ...
        let eq_constraint = solver.eq(a.clone(), a.clone());
    ...
    pub fn test_numerical_inbound<S: Solver>(solver: &mut S) {
        ...
        let eq_constraint = solver.eq(a.clone(), b.clone());
        solver.assert(eq_constraint).unwrap();
        assert_eq!(solver.check().unwrap(), Response::Sat);
        assert_eq!(
    ...
    pub fn test_extract<S: Solver>(solver: &mut S) {
        ...
        let eq_constraint = solver.eq(a.clone(), c);
        solver.assert(eq_constraint).unwrap();
        assert_eq!(solver.check().unwrap(), Response::Sat);
        assert_eq!(
    ...
    pub fn assert_solver_eq<S: Solver>(solver: &mut S, lhs: BitVec<S::Rep>, rhs: BitVec<S::Rep>) {
        solver.push().unwrap();
        let eq_constraint = solver.eq(lhs.clone(), rhs.clone());
        solver.assert(eq_constraint.clone()).unwrap();
        assert_eq!(solver.check().unwrap(), Response::Sat);
        solver.pop().unwrap();
        solver.push().unwrap();
        let ne_constraint = solver.ne(lhs, rhs);
        solver.assert(ne_constraint.clone()).unwrap();
        ...
        ($fn_name:ident,
         ...
         $cond_val:expr, $then_val:expr, $else_val:expr, $expected_val:expr $(,)?) => {
            ...
            fn $fn_name() {
                ...
                let actual = solver.ite(cond, then_val.clone(), else_val.clone());
