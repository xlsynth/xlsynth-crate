...
pub struct EasySMTSolver {
    ...
    solver_fn: SolverFn,
    declared: HashMap<String, BitVec<SExpr>>,
}

impl EasySMTSolver {
    pub fn new(config: EasySMTConfig) -> io::Result<EasySMTSolver> {
    ...
    fn bv_to_bool(&self, context: &Context, value: BitVec<SExpr>) -> SExpr {
        match value {
            BitVec::BitVec { rep, width: 1 } => context.eq(rep, context.binary(1, 1)),
            _ => panic!(
        ...
        F: Fn(&Context, SExpr, SExpr) -> SExpr,
    {
        ...
        match (lhs, rhs) {
            (BitVec::BitVec { width: w1, rep: r1 }, BitVec::BitVec { width: w2, rep: r2 }) => {
                ...
                self.bool_to(&context, op(&context, r1, r2))
            }
...
impl Solver for EasySMTSolver {
    ...
    fn declare(&mut self, name: &str, width: usize) -> io::Result<BitVec<SExpr>> {
        if width == 0 {
            self.declared.insert(name.to_string(), BitVec::ZeroWidth);
        ...
        self.declared.insert(name.to_string(), ret.clone());
        Ok(ret)
    }
    ...
    fn get_value(&mut self, bit_vec: BitVec<SExpr>, ty: &ir::Type) -> io::Result<xlsynth::IrValue> {
        let shared = Arc::clone(&self.context);
        let mut context = shared.lock().unwrap();
        match bit_vec {
            BitVec::BitVec { rep, .. } => {
                let value = context.get_value(vec![rep])?[0].1;
                let atom = context.get_atom(value).expect("model value must be atom");
                let lit_str = if let Some(rest) = atom.strip_prefix("#b") {
                    format!("0b{}", rest)
                ...
                IrValue::parse_typed(&format!("{}:{}", ty, lit_str))
        ...
        match bit_vec {
            BitVec::BitVec { width, rep } => {
                ...
                BitVec::BitVec {
                    ...
                    rep: context.extract(high, low, rep),
                }
            }
    ...
    fn concat(&mut self, lhs: BitVec<SExpr>, rhs: BitVec<SExpr>) -> BitVec<SExpr> {
        ...
        match (&lhs, &rhs) {
            ...
            (BitVec::ZeroWidth, _) => rhs,
            (_, BitVec::ZeroWidth) => lhs,
        }
    }
    ...
    fn ite(
        ...
        match (lhs.clone(), then, else_) {
            (
                ...
                BitVec::BitVec {
                    width: w2,
                    rep: context.ite(self.bv_to_bool(&context, lhs), r2, r3),
                }
            }
...
test_solver!(
    boolector_tests,
    super::EasySMTSolver::new(super::EasySMTConfig {
        replay_file: Some("boolector_replay.smt2".into()),
        ..super::EasySMTConfig::boolector()
    })
    .unwrap()
);
