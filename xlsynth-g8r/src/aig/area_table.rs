...
/// Returns the textual identifier and type for an operand node reference.
fn typed_operand_text(f: &ir::Fn, node_ref: ir::NodeRef) -> String {
    let node = f.get_node(node_ref);
    match &node.payload {
        ir::NodePayload::Literal(value) => format!(
            "{}: {}",
            ir::node_textual_id(f, node_ref),
            value.to_string_fmt(IrFormatPreference::Hex).unwrap()
        ),
        _ => format!("{}: {}", ir::node_textual_id(f, node_ref), node.ty),
    }
}

/// Formats a list of operand node references with textual ids and types.
fn typed_operands_text(f: &ir::Fn, node_refs: &[ir::NodeRef]) -> String {
    node_refs
        .iter()
        .map(|node_ref| typed_operand_text(f, *node_ref))
        .collect::<Vec<_>>()
        .join(", ")
}
...
fn render_ir_node_text(f: &ir::Fn, node: &ir::Node) -> Option<String> {
    ...
    let payload = match &node.payload {
        ir::NodePayload::Nil | ir::NodePayload::GetParam(_) => return None,
        ir::NodePayload::Tuple(nodes) => format!("tuple({})", typed_operands_text(f, nodes)),
        ir::NodePayload::Array(nodes) => format!("array({})", typed_operands_text(f, nodes)),
        ir::NodePayload::ArraySlice {
            ...
            "array_slice({}, {}, width={})",
            typed_operand_text(f, *array),
            typed_operand_text(f, *start),
            width
        ),
        ir::NodePayload::TupleIndex { tuple, index } => format!(
            "tuple_index({}, index={})",
            typed_operand_text(f, *tuple),
            index
        ),
        ir::NodePayload::Binop(op, lhs, rhs) => format!(
            "{}({}, {})",
            ir::binop_to_operator(*op),
            typed_operand_text(f, *lhs),
            typed_operand_text(f, *rhs)
        ),
        ir::NodePayload::ExtCarryOut { lhs, rhs, c_in } => format!(
            "ext_carry_out({}, {}, {})",
            typed_operand_text(f, *lhs),
            typed_operand_text(f, *rhs),
            typed_operand_text(f, *c_in)
        ),
        ir::NodePayload::ExtPrioEncode { arg, lsb_prio } => format!(
            "ext_prio_encode({}, lsb_prio={})",
            typed_operand_text(f, *arg),
            lsb_prio
        ),
        ir::NodePayload::ExtNaryAdd { terms, arch } => {
            ...
            if !terms.is_empty() {
                attrs.push(
                    terms
                        .iter()
                        .map(|term| typed_operand_text(f, term.operand))
                        .collect::<Vec<_>>()
                        .join(", "),
                );
            }
        ...
        ir::NodePayload::Unop(op, arg) => format!(
            "{}({})",
            ir::unop_to_operator(*op),
            typed_operand_text(f, *arg)
        ),
        ir::NodePayload::Literal(value) => format!(
            "literal(value={})",
        ...
        ir::NodePayload::SignExt { arg, new_bit_count } => format!(
            "sign_ext({}, new_bit_count={})",
            typed_operand_text(f, *arg),
            new_bit_count
        ),
        ir::NodePayload::ZeroExt { arg, new_bit_count } => format!(
        ...
        ir::NodePayload::ArrayUpdate {
            ...
            let mut attrs = vec![
                typed_operand_text(f, *array),
                typed_operand_text(f, *value),
                format!("indices=[{}]", typed_operands_text(f, indices)),
            ];
            if *assumed_in_bounds {
                attrs.push("assumed_in_bounds=true".to_string());
            }
            format!("array_update({})", attrs.join(", "))
        }
        ir::NodePayload::ArrayIndex {
            ...
            format!("array_index({})", attrs.join(", "))
        }
        ir::NodePayload::DynamicBitSlice { arg, start, width } => format!(
            "dynamic_bit_slice({}, {}, width={})",
            typed_operand_text(f, *arg),
            typed_operand_text(f, *start),
            width
        ),
        ir::NodePayload::BitSlice { arg, start, width } => format!(
            "bit_slice({}, start={}, width={})",
            typed_operand_text(f, *arg),
            start,
            width
        ),
        ir::NodePayload::BitSliceUpdate {
            ...
            typed_operand_text(f, *arg),
            typed_operand_text(f, *start),
            typed_operand_text(f, *update_value)
        ),
        ir::NodePayload::Assert {
            ...
            typed_operand_text(f, *token),
            typed_operand_text(f, *activate),
            message,
            label
        ),
        ir::NodePayload::Trace {
            ...
            "trace({}, {}, format={:?}, data_operands=[{}])",
            typed_operand_text(f, *token),
            typed_operand_text(f, *activated),
            format,
            typed_operands_text(f, operands)
        ),
        ir::NodePayload::InstantiationInput {
            ...
            "instantiation_input({}, instantiation={}, port_name={})",
            typed_operand_text(f, *arg),
            instantiation,
            port_name
        ),
        ir::NodePayload::InstantiationOutput {
            ...
            "instantiation_output(instantiation={}, port_name={})",
            instantiation, port_name
        ),
        ir::NodePayload::RegisterRead { register } => {
            format!("register_read(register={})", register)
        }
        ir::NodePayload::RegisterWrite {
            ...
            if let Some(load_enable) = load_enable {
                parts.push(format!(
                    "load_enable={}",
                    typed_operand_text(f, *load_enable)
                ));
            }
            if let Some(reset) = reset {
                parts.push(format!("reset={}", typed_operand_text(f, *reset)));
            }
            format!("register_write({})", parts.join(", "))
        }
        ir::NodePayload::AfterAll(nodes) => format!("after_all({})", typed_operands_text(f, nodes)),
        ir::NodePayload::Nary(op, nodes) => format!(
            "{}({})",
            ir::nary_op_to_operator(*op),
            typed_operands_text(f, nodes)
        ),
        ir::NodePayload::Invoke { to_apply, operands } => {
            if operands.is_empty() {
                format!("invoke(to_apply={})", to_apply)
            } else {
                format!(
                    "invoke({}, to_apply={})",
                    typed_operands_text(f, operands),
        ...
        ir::NodePayload::PrioritySel {
            ...
            let mut attrs = vec![
                typed_operand_text(f, *selector),
                format!("cases=[{}]", typed_operands_text(f, cases)),
            ];
            if let Some(default) = default {
                attrs.push(format!("default={}", typed_operand_text(f, *default)));
            }
            format!("priority_sel({})", attrs.join(", "))
        }
        ir::NodePayload::OneHotSel { selector, cases } => format!(
            "one_hot_sel({}, cases=[{}])",
            typed_operand_text(f, *selector),
            typed_operands_text(f, cases)
        ),
        ir::NodePayload::OneHot { arg, lsb_prio } => format!(
            "one_hot({}, lsb_prio={})",
            typed_operand_text(f, *arg),
            lsb_prio
        ),
        ir::NodePayload::Sel {
            ...
            format!("sel({})", attrs.join(", "))
        }
        ir::NodePayload::Cover { predicate, label } => format!(
            "cover({}, label={:?})",
            typed_operand_text(f, *predicate),
            label
        ),
        ir::NodePayload::Decode { arg, width } => {
            format!("decode({}, width={})", typed_operand_text(f, *arg), width)
        }
        ir::NodePayload::Encode { arg } => format!("encode({})", typed_operand_text(f, *arg)),
        ir::NodePayload::CountedFor {
            ...
            if !invariant_args.is_empty() {
                attrs.push(format!(
                    "invariant_args=[{}]",
                    typed_operands_text(f, invariant_args)
                ));
            }
            format!("counted_for({})", attrs.join(", "))
        }
    };

    Some(format!("{}: {} = {}", name, node.ty, payload))
}
