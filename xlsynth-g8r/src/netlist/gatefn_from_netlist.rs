...
mod tests {
    ...
    #[test]
    fn test_multi_bit_literal_tie_off_all_zero() {
        // Build a netlist and Liberty proto for a buffer with a multi-bit literal
        // tie-off to 0
        let mut interner: StringInterner<StringBackend<SymbolU32>> = StringInterner::new();
        let a = interner.get_or_intern("a");
        let n = interner.get_or_intern("n");
        ...
        let instances = vec![NetlistInstance {
            ...
            connections: vec![
                ...
                (
                    interner.get_or_intern("Y"),
                    NetRef::Literal(xlsynth::IrBits::make_ubits(4, 0).unwrap()),
                ),
            ],
            inst_lineno: 0,
            inst_colno: 0,
        ...
        .expect("Multi-bit literal tie-off should be supported");
        let s = gate_fn.to_string();
        // n should be assigned to a zero node (literal false)
        assert!(
            s.contains("literal(false)") || s.contains("= %0"),
            "GateFn output: {}",
            s
    ...
    #[test]
    fn test_multi_bit_literal_tie_off_all_one() {
        // Build a netlist and Liberty proto for a buffer with a multi-bit literal
        // tie-off to 1
        let mut interner: StringInterner<StringBackend<SymbolU32>> = StringInterner::new();
        let a = interner.get_or_intern("a");
        ...
        let instances = vec![NetlistInstance {
            ...
            connections: vec![
                ...
                (
                    interner.get_or_intern("Y"),
                    NetRef::Literal(xlsynth::IrBits::make_ubits(4, 1).unwrap()),
                ),
            ],
            inst_lineno: 0,
            inst_colno: 0,
        ...
        .expect("Multi-bit literal tie-off should be supported");
        let s = gate_fn.to_string();
        // n should be assigned to a one node (literal true)
        assert!(
            s.contains("literal(true)") || s.contains("= %1"),
            "GateFn output: {}",
            s
        );
    }
    ...
    #[test]
    fn test_instance_input_tied_to_multibit_all_zero_literal() {
        // Cell with single boolean function Y=A; tie A to 2'b00 -> false
        let mut interner: StringInterner<StringBackend<SymbolU32>> = StringInterner::new();
        let y = interner.get_or_intern("y");
        let buf = interner.get_or_intern("BUFX1");
        let u1 = interner.get_or_intern("u1");
        ...
        let instances = vec![NetlistInstance {
            ...
            connections: vec![
                (
                    interner.get_or_intern("A"),
                    NetRef::Literal(xlsynth::IrBits::make_ubits(2, 0).unwrap()),
                ),
    ...
    #[test]
    fn test_instance_input_tied_to_multibit_all_one_literal() {
        // Cell with single boolean function Y=A; tie A to 3'b111 -> true
        let mut interner: StringInterner<StringBackend<SymbolU32>> = StringInterner::new();
        let y = interner.get_or_intern("y");
        let buf = interner.get_or_intern("BUFX1");
        let u1 = interner.get_or_intern("u1");
        ...
        let instances = vec![NetlistInstance {
            ...
            connections: vec![
                (
                    interner.get_or_intern("A"),
                    NetRef::Literal(xlsynth::IrBits::make_ubits(3, 0b111).unwrap()),
                ),
                (interner.get_or_intern("Y"), NetRef::Simple(NetIndex(0))),
            ],
            inst_lineno: 0,
        ...
        .unwrap();
        let s = gate_fn.to_string();
        // True is represented by not(%0) in our AIG
        assert!(
            s.contains("not(%0)") || s.contains("= not(%0)"),
            "GateFn output: {}",
            s
        );
    }
