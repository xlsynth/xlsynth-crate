...
fn has_boundary_output(f: &IrFn, sel: &HashSet<usize>) -> bool {
    ...
    for &idx in sel.iter() {
        if let Some(us) = users.get(&idx) {
            if us.iter().any(|u| !sel.contains(u)) {
                return true;
            }
        }
    }
...
fuzz_target!(|sample: FuzzSample| {
    ...
    let param_mode = if (h & 1) == 0 {
        ...
        ParamOrderMode::NonDefault
    };
    let ret_mode = if (h & 2) == 0 {
        ReturnOrderMode::Default
    } else {
        ReturnOrderMode::NonDefault
    };
    ...
    if let ParamOrderMode::NonDefault = param_mode {
        permute_vec_deterministic(&mut ordering.params, h.rotate_right(17));
    }
    if let ReturnOrderMode::NonDefault = ret_mode {
        permute_vec_deterministic(&mut ordering.returns, h.rotate_left(9));
    }
    ...
    #[cfg(feature = "has-bitwuzla")]
    {
        ...
        } = r
        {
            ...
            if let Some(PackageMember::Function(f)) = work_pkg
                .members
                .iter()
                .find(|m| matches!(m, PackageMember::Function(f) if f.name == "outlined_outer"))
            {
                log::info!("Outlined outer IR fn:\n{}", f);
            }
            panic!(
                "Outline equivalence failed (Bitwuzla): lhs_inputs={:?} rhs_inputs={:?}",
    ...
    #[cfg(feature = "has-boolector")]
    {
        let r = prove_ir_fn_equiv::<Boolector>(
            &BoolectorConfig::new(),
            &lhs,
            &rhs,
            AssertionSemantics::Same,
        ...
        } = r
        {
            ...
            panic!(
                "Outline equivalence failed (Boolector): lhs_inputs={:?} rhs_inputs={:?}",
                lhs_inputs, rhs_inputs
            );
        }
    }
