...
fuzz_target!(|pair: FuzzSampleSameTypedPair| {
    ...
    // 5) Rebase desired onto orig and prove semantic equivalence: rebase_onto(orig, desired) == desired
    let mut next_id = max_text_id(orig) + 1;
    let rebased: ir::Fn = rebase_onto(
        &desired,
        &orig,
        "rebased",
        || {
    ...
    if let Err(e) = validate_fn(&rebased, &pkg) {
        panic!(
            "rebased IR failed composite validation: {}",
            e
        );
    }
    ...
    match prove_equiv_via_toolchain::prove_ir_fn_equiv_via_toolchain(&desired, &rebased) {
        EquivResult::Proved => {}
        other => {
            // Treat tool infra failure as non-sample failure; but equivalence disproved must panic.
            match other {
                EquivResult::OtherProcessError(_) => return,
