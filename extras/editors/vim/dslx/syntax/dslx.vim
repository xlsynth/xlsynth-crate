...
" DSLX comments are line comments only in the frontend scanner.
syn keyword dslxTodo TODO FIXME XXX NOTE contained
syn match dslxComment "//.*$" contains=dslxTodo,@Spell

" Attributes such as #[test] and #![feature(use_syntax)].
syn region dslxAttribute start=+#!\=\[+ skip=+"\([^"\\]\|\\.\)*"\|`[^`]*`\|'\([^'\\]\|\\.\)'+ end=+\]+ contains=dslxComment,dslxString,dslxBacktickString,dslxCharacter,dslxNumber,dslxBoolean,dslxKeyword,dslxType,dslxBuiltin,dslxOperator
...
syn keyword dslxKeyword as const else enum fn for if impl import in let match out proc pub self spawn struct use
syn match dslxKeyword "\%(^\|[^A-Za-z0-9_']\)\zsunroll_for!\ze\%([^A-Za-z0-9_']\|$\)"
...
syn keyword dslxType bits token uN sN xN bool chan Self type
...
syn match dslxBuiltin "\%(^\|[^A-Za-z0-9_']\)\zs\%(cover\|fail\|assert\|trace\|all_ones\|zero\|trace_fmt\|assert_fmt\|vtrace_fmt\|gate\|const_assert\)!\ze\%([^A-Za-z0-9_']\|$\)"
