...
fn enum_as_tups(enum_def: &dslx::EnumDef, type_info: &dslx::TypeInfo) -> Vec<(String, IrValue)> {
    ...
    for i in 0..enum_def.get_member_count() {
        ...
        let member_const = type_info.get_const_expr(member_expr).expect("enum values should be constexpr");
...
fn convert_enum_to_rust(enum_def: &dslx::EnumDef, type_info: &dslx::TypeInfo) -> Result<String, XlsynthError> {
    ...
    for (member_name, value) in tups.iter() {
        ...
        lines.push(format!("            {}::{} => IrValue::make_bits({}, {}).unwrap(),", enum_name, member_name, value.bit_count(), value_str));
    }
...
pub fn convert_leaf_module(import_data: &mut dslx::ImportData, dslx_program: &str, path: &std::path::Path) -> Result<String, XlsynthError> {
    // If the path is `path/to/foo.x` then the module name is `foo`.
    let module_name = path.file_stem().unwrap().to_str().unwrap();
    let path_str = path.to_str().unwrap();
    let typechecked_module = dslx::parse_and_typecheck(dslx_program, path_str, module_name, import_data)?;
    ...
    for i in 0..module.get_type_definition_count() {
        ...
        match type_def_kind {
            ...
            dslx::TypeDefinitionKind::StructDef => todo!("convert struct definition from DSLX to Rust"),
...
mod tests {
    ...
    fn test_convert_leaf_module_enum_def_only() {
        ...
        assert_eq!(rust, r#"pub enum MyEnum {
    A = 0,
    B = 3,
}
