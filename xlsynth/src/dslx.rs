...
impl ImportData {
    ...
    pub fn new(
        ...
        let additional_search_paths_c_str: Vec<std::ffi::CString> = additional_search_paths
            .iter()
            .map(|p| std::ffi::CString::new(p.to_str().unwrap()).unwrap())
            .collect::<Vec<_>>();
        let additional_search_paths_c_ptr: Vec<*const std::os::raw::c_char> =
            additional_search_paths_c_str
                .iter()
                .map(|s| s.as_ptr())
                .collect::<Vec<_>>();
