...
/// Generated typed DSLX AOT package for nested module paths and repeated top
/// basenames.
pub mod namespaced_widget_package_aot {
    include!(env!("XLSYNTH_AOT_NAMESPACED_WIDGET_PACKAGE_RS"));
}
...
mod tests {
    ...
    // Verifies: package-generated modules preserve nested DSLX paths even when
    ...
    fn package_builder_supports_nested_modules_with_same_basename_tops() -> Result<(), AotError> {
        ...
        let input = Widget { widget_id: ub(7) };
        let mut echo_runner = aot_namespaced_echo_widget::new_runner()?;
        let echoed = echo_runner.run(&input)?;
        let mut bump_runner = aot_namespaced_bump_widget::new_runner()?;
        let bumped = bump_runner.run(&echoed)?;
