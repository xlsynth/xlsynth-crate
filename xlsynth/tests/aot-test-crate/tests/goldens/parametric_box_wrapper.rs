...
pub mod parametric_box {
    ...
    unsafe extern "C" {
        #[link_name = "__xlsynth_standalone_parametric_box"]
    ...
    pub const ARTIFACT_METADATA: AotArtifactMetadata = AotArtifactMetadata {
        abi_version: 1,
        entrypoint_symbol: "__xlsynth_standalone_parametric_box",
        has_asserts: false,
        has_traces: false,
    };
    ...
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn decode_output_0(src: &[u8]) -> Result<Box8, AotError> {
    ...
    impl Runner {
            ...
            Ok(Self {
                inner: unsafe {
                    StandaloneRunner::new(
                        &ARTIFACT_METADATA,
                        __xlsynth_aot_linked_symbol_parametric_box as *const (),
                        &RUNNER_LAYOUT,
                    )?
                },
            })
        ...
        pub fn run_with_events(
            &mut self,
            x: &Box8,
        ) -> Result<AotRunResult<Box8>, AotError> {
        ...
        pub fn run(&mut self, x: &Box8) -> Result<Box8, AotError> {
            encode_input_0(x, self.inner.input_mut(0))?;
            self.inner.run_raw();
