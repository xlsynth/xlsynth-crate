...
pub mod self_alias_widget {
    ...
    unsafe extern "C" {
        #[link_name = "__self_alias_widget__echo_widget"]
        fn __xlsynth_aot_linked_symbol_self_alias_widget();
    }

    const ENTRYPOINTS_PROTO: &[u8] = include_bytes!("self_alias_widget.entrypoints.pb");
    ...
    fn encode_input_0(_value: &WidgetHandle, dst: &mut [u8]) -> Result<(), xlsynth::XlsynthError> {
    ...
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn decode_output_0(src: &[u8]) -> Result<WidgetHandle, xlsynth::XlsynthError> {
    ...
    pub fn descriptor() -> xlsynth::AotEntrypointDescriptor<'static> {
        unsafe {
            xlsynth::AotEntrypointDescriptor::from_raw_parts_unchecked(
                ENTRYPOINTS_PROTO,
                __xlsynth_aot_linked_symbol_self_alias_widget as *const () as usize,
                xlsynth::AotEntrypointMetadata {
                    symbol: "__self_alias_widget__echo_widget".to_string(),
    ...
    impl Runner {
        ...
        pub fn run_with_events(
            &mut self,
            widget: &WidgetHandle,
        ) -> Result<xlsynth::AotRunResult<WidgetHandle>, xlsynth::XlsynthError> {
        ...
        pub fn run(
            &mut self,
            widget: &WidgetHandle,
        ) -> Result<WidgetHandle, xlsynth::XlsynthError> {
            encode_input_0(widget, self.inner.input_mut(0))?;
            self.inner.run()?;
