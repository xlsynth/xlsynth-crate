// SPDX-License-Identifier: Apache-2.0
// Generated by xlsynth::aot_builder from DSLX build spec "parametric_imports".

pub use xlsynth_aot_runtime::{AotArtifactMetadata, AotError, AotRunResult, SBits, UBits};
#[allow(unused_imports)]
use xlsynth_aot_runtime::{
    read_leaf_element, write_leaf_element, AotElementLayout, AotRunnerLayout,
    StandaloneRunner,
};
pub mod parametric_imports {
    #![allow(dead_code)]
    #![allow(unused_imports)]
    use super::{
        read_leaf_element, write_leaf_element, AotArtifactMetadata, AotElementLayout,
        AotError, AotRunResult, AotRunnerLayout, SBits, StandaloneRunner, UBits,
    };
    #[allow(non_camel_case_types)]
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct Mixed__N_4 {
        pub mode: LocalMode,
        pub remote: super::parametric_lib::RemotePlain,
        pub nested: [[UBits<8>; 4]; 2],
        pub boxes: [Box__N_8; 2],
        pub remote_boxes: [super::parametric_lib::RemoteBox__N_8; 2],
    }
    #[allow(non_camel_case_types)]
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct Box__N_8 {
        pub value: UBits<8>,
    }
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    pub enum LocalMode {
        Idle = 0,
        Busy = 1,
    }
    pub type Mixed4 = Mixed__N_4;
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct ParametricImportsResult {
        pub mixed: Mixed4,
        pub imported_direct: super::parametric_lib::RemoteBox__N_8,
        pub imported_pair: super::parametric_lib::RemotePair__A_8__B_23,
    }
    unsafe extern "C" {
        #[link_name = "__xlsynth_standalone_parametric_imports"]
        fn __xlsynth_aot_linked_symbol_parametric_imports();
    }
    const INPUT_BUFFER_SIZES: &[usize] = &[14, 1, 8];
    const INPUT_BUFFER_ALIGNMENTS: &[usize] = &[8, 8, 8];
    const OUTPUT_BUFFER_SIZES: &[usize] = &[24];
    const OUTPUT_BUFFER_ALIGNMENTS: &[usize] = &[8];
    const RUNNER_LAYOUT: AotRunnerLayout = AotRunnerLayout {
        input_buffer_sizes: INPUT_BUFFER_SIZES,
        input_buffer_alignments: INPUT_BUFFER_ALIGNMENTS,
        output_buffer_sizes: OUTPUT_BUFFER_SIZES,
        output_buffer_alignments: OUTPUT_BUFFER_ALIGNMENTS,
        temp_buffer_size: 0,
        temp_buffer_alignment: 1,
    };
    /// Runtime-visible metadata for the generated standalone artifact.
    pub const ARTIFACT_METADATA: AotArtifactMetadata = AotArtifactMetadata {
        abi_version: 1,
        entrypoint_symbol: "__xlsynth_standalone_parametric_imports",
        has_asserts: false,
        has_traces: false,
    };
    const INPUT0_LAYOUT: &[AotElementLayout] = &[
        AotElementLayout {
            offset: 0,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 1,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 2,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 3,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 4,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 5,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 6,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 7,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 8,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 9,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 10,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 11,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 12,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 13,
            data_size: 1,
            padded_size: 1,
        },
    ];
    const INPUT1_LAYOUT: &[AotElementLayout] = &[
        AotElementLayout {
            offset: 0,
            data_size: 1,
            padded_size: 1,
        },
    ];
    const INPUT2_LAYOUT: &[AotElementLayout] = &[
        AotElementLayout {
            offset: 0,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 4,
            data_size: 3,
            padded_size: 4,
        },
    ];
    const OUTPUT0_LAYOUT: &[AotElementLayout] = &[
        AotElementLayout {
            offset: 0,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 1,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 2,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 3,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 4,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 5,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 6,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 7,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 8,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 9,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 10,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 11,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 12,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 13,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 14,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 16,
            data_size: 1,
            padded_size: 1,
        },
        AotElementLayout {
            offset: 20,
            data_size: 3,
            padded_size: 4,
        },
    ];
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn encode_input_0(_value: &Mixed4, dst: &mut [u8]) -> Result<(), AotError> {
        debug_assert_eq!(dst.len(), 14);
        dst.fill(0);
        let encoded_value: u64 = match (*_value).mode {
            LocalMode::Idle => 0,
            LocalMode::Busy => 1,
        };
        let encoded_bits = UBits::<2>::from_u64(encoded_value)?;
        let encoded_bytes = encoded_bits.to_le_bytes()?;
        write_leaf_element(dst, &INPUT0_LAYOUT[0usize], &encoded_bytes);
        let encoded_bytes = (((*_value).remote).id).to_le_bytes()?;
        write_leaf_element(dst, &INPUT0_LAYOUT[0usize + 1], &encoded_bytes);
        for index_0 in 0..2 {
            for index_1 in 0..4 {
                let encoded_bytes = ((((*_value).nested)[index_0])[index_1])
                    .to_le_bytes()?;
                write_leaf_element(
                    dst,
                    &INPUT0_LAYOUT[0usize + 2 + index_0 * 4 + index_1],
                    &encoded_bytes,
                );
            }
        }
        for index_2 in 0..2 {
            let encoded_bytes = ((((*_value).boxes)[index_2]).value).to_le_bytes()?;
            write_leaf_element(
                dst,
                &INPUT0_LAYOUT[0usize + 10 + index_2],
                &encoded_bytes,
            );
        }
        for index_3 in 0..2 {
            let encoded_bytes = ((((*_value).remote_boxes)[index_3]).value)
                .to_le_bytes()?;
            write_leaf_element(
                dst,
                &INPUT0_LAYOUT[0usize + 12 + index_3],
                &encoded_bytes,
            );
        }
        debug_assert_eq!(INPUT0_LAYOUT.len(), 14);
        Ok(())
    }
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn encode_input_1(
        _value: &super::parametric_lib::RemoteBox__N_8,
        dst: &mut [u8],
    ) -> Result<(), AotError> {
        debug_assert_eq!(dst.len(), 1);
        dst.fill(0);
        let encoded_bytes = ((*_value).value).to_le_bytes()?;
        write_leaf_element(dst, &INPUT1_LAYOUT[0usize], &encoded_bytes);
        debug_assert_eq!(INPUT1_LAYOUT.len(), 1);
        Ok(())
    }
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn encode_input_2(
        _value: &super::parametric_lib::RemotePair__A_8__B_23,
        dst: &mut [u8],
    ) -> Result<(), AotError> {
        debug_assert_eq!(dst.len(), 8);
        dst.fill(0);
        let encoded_bytes = ((*_value).left).to_le_bytes()?;
        write_leaf_element(dst, &INPUT2_LAYOUT[0usize], &encoded_bytes);
        let encoded_bytes = ((*_value).right).to_le_bytes()?;
        write_leaf_element(dst, &INPUT2_LAYOUT[0usize + 1], &encoded_bytes);
        debug_assert_eq!(INPUT2_LAYOUT.len(), 2);
        Ok(())
    }
    #[allow(clippy::deref_addrof, clippy::explicit_auto_deref, clippy::identity_op)]
    fn decode_output_0(src: &[u8]) -> Result<ParametricImportsResult, AotError> {
        debug_assert_eq!(src.len(), 24);
        let mut decoded_bytes_0 = vec![0u8; 1];
        read_leaf_element(src, &OUTPUT0_LAYOUT[0usize], &mut decoded_bytes_0);
        let decoded_bits_1 = UBits::<2>::from_le_bytes(&decoded_bytes_0)?;
        let decoded_scalar_2 = decoded_bits_1.to_u64()?;
        let decoded_value_3 = match decoded_scalar_2 {
            0 => LocalMode::Idle,
            1 => LocalMode::Busy,
            value => {
                return Err(
                    AotError(
                        format!("AOT decode invalid enum value {value} for LocalMode"),
                    ),
                );
            }
        };
        let mut decoded_bytes_4 = vec![0u8; 1];
        read_leaf_element(src, &OUTPUT0_LAYOUT[0usize + 1], &mut decoded_bytes_4);
        let decoded_value_5 = UBits::<8>::from_le_bytes(&decoded_bytes_4)?;
        let decoded_value_6 = super::parametric_lib::RemotePlain {
            id: decoded_value_5,
        };
        let mut decoded_items_8 = Vec::with_capacity(2);
        for index_0 in 0..2 {
            let mut decoded_items_10 = Vec::with_capacity(4);
            for index_1 in 0..4 {
                let mut decoded_bytes_11 = vec![0u8; 1];
                read_leaf_element(
                    src,
                    &OUTPUT0_LAYOUT[0usize + 2 + index_0 * 4 + index_1],
                    &mut decoded_bytes_11,
                );
                let decoded_value_12 = UBits::<8>::from_le_bytes(&decoded_bytes_11)?;
                decoded_items_10.push(decoded_value_12);
            }
            let decoded_value_9: [UBits<8>; 4] = match std::convert::TryInto::try_into(
                decoded_items_10,
            ) {
                Ok(value) => value,
                Err(values) => {
                    return Err(
                        AotError(
                            format!(
                                "AOT decode internal error: expected 4 array elements, got {}",
                                values.len()
                            ),
                        ),
                    );
                }
            };
            decoded_items_8.push(decoded_value_9);
        }
        let decoded_value_7: [[UBits<8>; 4]; 2] = match std::convert::TryInto::try_into(
            decoded_items_8,
        ) {
            Ok(value) => value,
            Err(values) => {
                return Err(
                    AotError(
                        format!(
                            "AOT decode internal error: expected 2 array elements, got {}",
                            values.len()
                        ),
                    ),
                );
            }
        };
        let mut decoded_items_14 = Vec::with_capacity(2);
        for index_2 in 0..2 {
            let mut decoded_bytes_15 = vec![0u8; 1];
            read_leaf_element(
                src,
                &OUTPUT0_LAYOUT[0usize + 10 + index_2],
                &mut decoded_bytes_15,
            );
            let decoded_value_16 = UBits::<8>::from_le_bytes(&decoded_bytes_15)?;
            let decoded_value_17 = Box__N_8 {
                value: decoded_value_16,
            };
            decoded_items_14.push(decoded_value_17);
        }
        let decoded_value_13: [Box__N_8; 2] = match std::convert::TryInto::try_into(
            decoded_items_14,
        ) {
            Ok(value) => value,
            Err(values) => {
                return Err(
                    AotError(
                        format!(
                            "AOT decode internal error: expected 2 array elements, got {}",
                            values.len()
                        ),
                    ),
                );
            }
        };
        let mut decoded_items_19 = Vec::with_capacity(2);
        for index_3 in 0..2 {
            let mut decoded_bytes_20 = vec![0u8; 1];
            read_leaf_element(
                src,
                &OUTPUT0_LAYOUT[0usize + 12 + index_3],
                &mut decoded_bytes_20,
            );
            let decoded_value_21 = UBits::<8>::from_le_bytes(&decoded_bytes_20)?;
            let decoded_value_22 = super::parametric_lib::RemoteBox__N_8 {
                value: decoded_value_21,
            };
            decoded_items_19.push(decoded_value_22);
        }
        let decoded_value_18: [super::parametric_lib::RemoteBox__N_8; 2] = match std::convert::TryInto::try_into(
            decoded_items_19,
        ) {
            Ok(value) => value,
            Err(values) => {
                return Err(
                    AotError(
                        format!(
                            "AOT decode internal error: expected 2 array elements, got {}",
                            values.len()
                        ),
                    ),
                );
            }
        };
        let decoded_value_23 = Mixed4 {
            mode: decoded_value_3,
            remote: decoded_value_6,
            nested: decoded_value_7,
            boxes: decoded_value_13,
            remote_boxes: decoded_value_18,
        };
        let mut decoded_bytes_24 = vec![0u8; 1];
        read_leaf_element(src, &OUTPUT0_LAYOUT[0usize + 14], &mut decoded_bytes_24);
        let decoded_value_25 = UBits::<8>::from_le_bytes(&decoded_bytes_24)?;
        let decoded_value_26 = super::parametric_lib::RemoteBox__N_8 {
            value: decoded_value_25,
        };
        let mut decoded_bytes_27 = vec![0u8; 1];
        read_leaf_element(src, &OUTPUT0_LAYOUT[0usize + 15], &mut decoded_bytes_27);
        let decoded_value_28 = UBits::<8>::from_le_bytes(&decoded_bytes_27)?;
        let mut decoded_bytes_29 = vec![0u8; 3];
        read_leaf_element(src, &OUTPUT0_LAYOUT[0usize + 15 + 1], &mut decoded_bytes_29);
        let decoded_value_30 = UBits::<23>::from_le_bytes(&decoded_bytes_29)?;
        let decoded_value_31 = super::parametric_lib::RemotePair__A_8__B_23 {
            left: decoded_value_28,
            right: decoded_value_30,
        };
        let decoded_value_32 = ParametricImportsResult {
            mixed: decoded_value_23,
            imported_direct: decoded_value_26,
            imported_pair: decoded_value_31,
        };
        debug_assert_eq!(OUTPUT0_LAYOUT.len(), 17);
        Ok(decoded_value_32)
    }
    /// Reusable runner for the generated typed DSLX AOT entrypoint.
    ///
    /// A runner caches the standalone ABI buffers; create one runner per
    /// concurrent caller instead of sharing it across threads.
    pub struct Runner {
        inner: StandaloneRunner,
    }
    impl Runner {
        /// Creates a reusable runner for the generated typed DSLX entrypoint.
        ///
        /// # Errors
        ///
        /// Returns an error if the standalone artifact metadata cannot initialize
        /// an AOT runner.
        pub fn new() -> Result<Self, AotError> {
            Ok(Self {
                inner: unsafe {
                    StandaloneRunner::new(
                        &ARTIFACT_METADATA,
                        __xlsynth_aot_linked_symbol_parametric_imports as *const (),
                        &RUNNER_LAYOUT,
                    )?
                },
            })
        }
        /// Runs the entrypoint and returns the output together with assertion
        /// events.
        ///
        /// # Errors
        ///
        /// Returns an error if input packing, AOT execution, or output decoding
        /// fails.
        pub fn run_with_events(
            &mut self,
            mixed: &Mixed4,
            imported_direct: &super::parametric_lib::RemoteBox__N_8,
            imported_pair: &super::parametric_lib::RemotePair__A_8__B_23,
        ) -> Result<AotRunResult<ParametricImportsResult>, AotError> {
            encode_input_0(mixed, self.inner.input_mut(0))?;
            encode_input_1(imported_direct, self.inner.input_mut(1))?;
            encode_input_2(imported_pair, self.inner.input_mut(2))?;
            self.inner.run_raw();
            Ok(AotRunResult {
                output: decode_output_0(self.inner.output(0))?,
                assert_messages: self.inner.assert_messages().to_vec(),
            })
        }
        /// Runs the entrypoint and returns only the decoded output value.
        ///
        /// # Errors
        ///
        /// Returns an error if input packing, AOT execution, output decoding, or
        /// an XLS assertion fails.
        pub fn run(
            &mut self,
            mixed: &Mixed4,
            imported_direct: &super::parametric_lib::RemoteBox__N_8,
            imported_pair: &super::parametric_lib::RemotePair__A_8__B_23,
        ) -> Result<ParametricImportsResult, AotError> {
            encode_input_0(mixed, self.inner.input_mut(0))?;
            encode_input_1(imported_direct, self.inner.input_mut(1))?;
            encode_input_2(imported_pair, self.inner.input_mut(2))?;
            self.inner.run_raw();
            if let Some(first_assertion) = self.inner.assert_messages().first() {
                return Err(AotError(format!("XLS assertion failed: {first_assertion}")));
            }
            decode_output_0(self.inner.output(0))
        }
    }
    /// Creates a reusable runner for the generated typed DSLX entrypoint.
    pub fn new_runner() -> Result<Runner, AotError> {
        Runner::new()
    }
}
pub mod parametric_lib {
    #![allow(dead_code)]
    #![allow(unused_imports)]
    use super::{
        read_leaf_element, write_leaf_element, AotArtifactMetadata, AotElementLayout,
        AotError, AotRunResult, AotRunnerLayout, SBits, StandaloneRunner, UBits,
    };
    #[allow(non_camel_case_types)]
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct RemoteBox__N_8 {
        pub value: UBits<8>,
    }
    #[allow(non_camel_case_types)]
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct RemotePair__A_8__B_23 {
        pub left: UBits<8>,
        pub right: UBits<23>,
    }
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct RemotePlain {
        pub id: UBits<8>,
    }
    pub type RemoteBox8 = RemoteBox__N_8;
}
