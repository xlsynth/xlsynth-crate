...
use xlsynth::{XlsIrBits, XlsynthError};

#[test]
fn arith_and_logic_ops() -> Result<(), XlsynthError> {
    let a = XlsIrBits::make_ubits(8, 0b0001_1010)?; // 26
    let b = XlsIrBits::make_ubits(8, 0b0000_0011)?; // 3
...
// Catches: byte-order regressions in `XlsIrBits::to_le_bytes`.
#[test]
fn conversions_shifts_and_slices() -> Result<(), XlsynthError> {
    let bits = XlsIrBits::make_sbits(8, -2)?;
    ...
    let wide = XlsIrBits::make_ubits(16, 0xABCD)?;
    assert_eq!(wide.to_le_bytes()?, vec![0xCD, 0xAB]);

    let from_bytes = XlsIrBits::from_le_bytes(12, &[0xCD, 0x0A])?;
    assert_eq!(from_bytes.get_bit_count(), 12);
    assert_eq!(from_bytes.to_le_bytes()?, vec![0xCD, 0x0A]);
    assert!(XlsIrBits::from_le_bytes(12, &[0xCD, 0xFA]).is_err());
...
fn comparison_methods_require_matching_widths() -> Result<(), XlsynthError> {
    let small = XlsIrBits::make_ubits(4, 0b0011)?;
    let medium = XlsIrBits::make_ubits(4, 0b0100)?;
    let wide = XlsIrBits::make_ubits(8, 0b0000_0100)?;
    ...
    let negative = XlsIrBits::make_sbits(4, -1)?;
    let positive = XlsIrBits::make_sbits(4, 1)?;
...
fn division_mod_and_construction_helpers() -> Result<(), XlsynthError> {
    assert_eq!(XlsIrBits::zero(8).to_u64()?, 0);
    assert_eq!(XlsIrBits::all_ones(8).to_u64()?, 0xFF);
    assert_eq!(XlsIrBits::signed_max_value(8).to_u64()?, 0x7F);
    assert_eq!(XlsIrBits::signed_min_value(8).to_u64()?, 0x80);
    assert!(!XlsIrBits::signed_max_value(8).is_negative());
    assert!(XlsIrBits::signed_min_value(8).is_negative());

    let lhs_u = XlsIrBits::make_ubits(8, 0xBC)?;
    let rhs_u = XlsIrBits::make_ubits(8, 0x07)?;
    assert_eq!(lhs_u.udiv(&rhs_u).to_u64()?, 0x1A);
    assert_eq!(lhs_u.umod(&rhs_u).to_u64()?, 0x06);
    assert_eq!(lhs_u.udiv(&XlsIrBits::zero(8)).to_u64()?, 0xFF);
    assert_eq!(lhs_u.umod(&XlsIrBits::zero(8)).to_u64()?, 0x00);

    let lhs_s = XlsIrBits::make_sbits(8, -68)?;
    let rhs_s = XlsIrBits::make_sbits(8, -7)?;
    assert_eq!(lhs_s.sdiv(&rhs_s).to_i64()?, 9);
    assert_eq!(lhs_s.smod(&rhs_s).to_i64()?, -5);
    assert_eq!(lhs_s.sdiv(&XlsIrBits::zero(8)).to_u64()?, 0x80);
    assert_eq!(lhs_s.smod(&XlsIrBits::zero(8)).to_u64()?, 0x00);
