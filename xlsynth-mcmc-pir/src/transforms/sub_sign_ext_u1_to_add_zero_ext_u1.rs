...
#[derive(Debug)]
pub struct SubSignExtU1ToAddZeroExtU1Transform;

impl SubSignExtU1ToAddZeroExtU1Transform {
...
impl PirTransform for SubSignExtU1ToAddZeroExtU1Transform {
    ...
    fn find_candidates(&mut self, f: &IrFn) -> Vec<TransformLocation> {
        ...
        for nr in f.node_refs() {
            match &f.get_node(nr).payload {
                ...
                NodePayload::Binop(Binop::Sub, x, sext) => {
                ...
                // Fold: add(x, zero_ext(b,w)) (either operand, zero_ext can also be
                // concat(0...,b)).
    ...
    fn apply(&self, f: &mut IrFn, loc: &TransformLocation) -> Result<(), String> {
        let target_ref = match loc {
            ...
            TransformLocation::RewireOperand { .. } => {
                return Err(
                    "SubSignExtU1ToAddZeroExtU1Transform: expected TransformLocation::Node, got RewireOperand"
                        .to_string(),
                );
            }
        };
        ...
        let w = Self::bits_width(f, target_ref).ok_or_else(|| {
            "SubSignExtU1ToAddZeroExtU1Transform: output must be bits[w]".to_string()
        })?;

        let payload = f.get_node(target_ref).payload.clone();
        match payload {
            // Expand: sub(x, sign_ext(b,w)) -> add(x, zero_ext(b,w))
            NodePayload::Binop(Binop::Sub, x, sext) => {
                if Self::bits_width(f, x) != Some(w) {
                    return Err(
                        "SubSignExtU1ToAddZeroExtU1Transform: expected sub lhs to be bits[w]"
                            .to_string(),
                    );
                }
                let Some((b_u1, sext_w)) = Self::sign_ext_u1_parts(f, sext) else {
                    return Err(
                        "SubSignExtU1ToAddZeroExtU1Transform: expected sub(x, sign_ext(b,w)) pattern"
                            .to_string(),
                    );
                };
                if sext_w != w {
                    return Err(
                        "SubSignExtU1ToAddZeroExtU1Transform: expected sign_ext new_bit_count to match output width"
                            .to_string(),
                    );
                }
                let zext = Self::mk_zero_ext_u1_node(f, w, b_u1);
                f.get_node_mut(target_ref).payload = NodePayload::Binop(Binop::Add, x, zext);
                Ok(())
            }

            // Fold: add(x, zero_ext(b,w)) -> sub(x, sign_ext(b,w))
            NodePayload::Binop(Binop::Add, a, b) => {
                ...
                let Some((x, b_u1)) = matched else {
                    return Err(
                        "SubSignExtU1ToAddZeroExtU1Transform: expected add(x, zero_ext(b,w)) pattern"
                            .to_string(),
                    );
                };

                let sext = Self::mk_sign_ext_u1_node(f, w, b_u1);
                f.get_node_mut(target_ref).payload = NodePayload::Binop(Binop::Sub, x, sext);
                Ok(())
            }
