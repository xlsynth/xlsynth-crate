            ...
            match &f.get_node(nr).payload {
                NodePayload::Nary(NaryOp::Concat, ops) => {
                    let sel_positions: Vec<usize> = ops
                        .iter()
                        .enumerate()
                        .filter_map(|(i, op)| matches!(f.get_node(*op).payload, NodePayload::Sel { .. }).then_some(i))
                        .collect();
                    ...
                    if default.is_some() || cases.len() != 2 || Self::bits_width(&f.get_node(*selector).ty) != Some(1) {
                        continue;
                    }
                ...
                NodePayload::Sel {
                    ...
                    let NodePayload::Nary(NaryOp::Concat, ops0) = f.get_node(cases[0]).payload.clone() else {
                        continue;
                    };
                    let NodePayload::Nary(NaryOp::Concat, ops1) = f.get_node(cases[1]).payload.clone() else {
                        continue;
                    };
                    if ops0.len() != ops1.len() {
                        continue;
                    }
                    let diff_positions: Vec<usize> = (0..ops0.len()).filter(|i| ops0[*i] != ops1[*i]).collect();
        ...
        match target_payload {
            NodePayload::Nary(NaryOp::Concat, ops) => {
                let sel_positions: Vec<usize> = ops
                    ...
                    .collect();
                if sel_positions.len() != 1 {
                    return Err("ConcatSelHoistTransform: expected exactly one sel operand".to_string());
                }
                ...
                let NodePayload::Sel {
                    ...
                    return Err("ConcatSelHoistTransform: sel operand must be 2-case/no-default".to_string());
                };
                if cases.len() != 2 {
                    return Err("ConcatSelHoistTransform: sel operand must have 2 cases".to_string());
                }
            ...
            NodePayload::Sel {
                ...
                let diff_positions: Vec<usize> = (0..ops0.len()).filter(|i| ops0[*i] != ops1[*i]).collect();
                if diff_positions.len() != 1 {
                    return Err("ConcatSelHoistTransform: expected exactly one varying concat slot".to_string());
                }
...
mod tests {
  ...
  ret out: bits[8] = sel(p, cases=[c0, c1], id=12)
        ...
        let target = match cand.clone() { TransformLocation::Node(nr) => nr, _ => unreachable!() };
        t.apply(&mut f, &cand).expect("apply");
        assert!(matches!(f.get_node(target).payload, NodePayload::Nary(NaryOp::Concat, _)));
    }
}
