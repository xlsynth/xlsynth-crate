...
#[derive(Debug)]
pub struct SmulSignExtU1ToSelNegTransform;

impl SmulSignExtU1ToSelNegTransform {
...
impl PirTransform for SmulSignExtU1ToSelNegTransform {
    ...
    fn find_candidates(&mut self, f: &IrFn) -> Vec<TransformLocation> {
        ...
        for nr in f.node_refs() {
            match &f.get_node(nr).payload {
                ...
                NodePayload::Binop(Binop::Smul, a, b) => {
    ...
    fn apply(&self, f: &mut IrFn, loc: &TransformLocation) -> Result<(), String> {
        let target_ref = match loc {
            ...
            TransformLocation::RewireOperand { .. } => {
                return Err(
                    "SmulSignExtU1ToSelNegTransform: expected TransformLocation::Node, got RewireOperand"
                        .to_string(),
                );
            }
        };
        ...
        let w = Self::bits_width(f, target_ref)
            .ok_or_else(|| "SmulSignExtU1ToSelNegTransform: output must be bits[w]".to_string())?;

        let payload = f.get_node(target_ref).payload.clone();
        match payload {
            // Expand: smul(x, sign_ext(b,w)) -> sel(b, [0, neg(x)])
            NodePayload::Binop(Binop::Smul, a, b) => {
                ...
                let Some((x, b_u1)) = matched else {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected smul(x, sign_ext(b,w)) pattern"
                            .to_string(),
                    );
                };
            ...
            // Fold: sel(b, [0, neg(x)]) -> smul(x, sign_ext(b,w))
            NodePayload::Sel {
                selector,
                cases,
                default,
                ...
                if cases.len() != 2 || default.is_some() {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected 2-case sel without default"
                            .to_string(),
                    );
                }
                if !Self::is_u1(f, selector) {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: selector must be bits[1]".to_string()
                    );
                }
                let zero = cases[0];
                let neg_x = cases[1];
                if !Self::is_zero_literal_node(f, zero, w) {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected sel case0 to be 0_w literal"
                            .to_string(),
                    );
                }
                if Self::bits_width(f, neg_x) != Some(w) {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected sel case1 to be bits[w]"
                            .to_string(),
                    );
                }
                let Some(x) = Self::neg_arg(f, neg_x) else {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected sel case1 to be neg(x)"
                            .to_string(),
                    );
                };
                if Self::bits_width(f, x) != Some(w) {
                    return Err(
                        "SmulSignExtU1ToSelNegTransform: expected x to be bits[w]".to_string()
                    );
                }

                let sext = Self::mk_sign_ext_u1_node(f, w, selector);
                f.get_node_mut(target_ref).payload = NodePayload::Binop(Binop::Smul, x, sext);
                Ok(())
            }
