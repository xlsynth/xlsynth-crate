...
use xlsynth_pir::ir::{self, PackageMember};
...
fuzz_target!(|sample: FuzzSample| {
    ...
    if generate_ir_fn(sample.ops.clone(), &mut pkg, None).is_err() {
        // Skip inputs that cannot construct a valid function.
        return;
    }
    ...
    // 4) Canonicalize the tool-emitted block by parsing as a package and
    //    re-emitting. The tool output is already a package.
    ...
    if generated_ir_text != roundtrip_ir_text {
        // Preserve the temporary directory for debugging and print its path.
        let keep_path = tmpdir.into_path();
        eprintln!(
