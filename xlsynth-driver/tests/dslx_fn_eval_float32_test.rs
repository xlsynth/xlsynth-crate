...
#[test]
fn test_dslx_fn_eval_float32_array_sum2() {
    let driver = env!("CARGO_BIN_EXE_xlsynth-driver");

    let tmp = xlsynth_test_helpers::make_test_tmpdir("dslx_fn_eval_f32_array_sum2");
    let dir = tmp.path();

    // DSLX module: sum two float32 values from a 2-element array.
    let dslx_path = dir.join("f32_array_sum2.x");
    let dslx_src = r#"
        import float32;
        fn sum2(xs: float32::F32[2]) -> float32::F32 {
          float32::add(xs[u32:0], xs[u32:1])
        }
    "#;
    std::fs::write(&dslx_path, dslx_src).expect("write dslx");

    // Inputs: [1.0, 1.0] -> 2.0. Unary param requires outer 1-tuple.
    let one = "(bits[1]:0, bits[8]:127, bits[23]:0)";
