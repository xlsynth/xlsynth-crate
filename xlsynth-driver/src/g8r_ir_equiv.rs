...
fn check_gate_fn_matches_function_type(
    ...
    if gate_fn.inputs.len() != function_type.param_types.len() {
        return Err(format!(
            "input count mismatch: gate_fn inputs={} rhs params={}",
            gate_fn.inputs.len(),
        ...
        .enumerate()
    {
        ...
        if input_width != param_width {
            return Err(format!(
                "input {} width mismatch: gate_fn bits[{}] rhs param bits[{}]",
...
pub fn handle_g8r_ir_equiv(matches: &clap::ArgMatches, config: &Option<ToolchainConfig>) {
    let g8r_path = Path::new(matches.get_one::<String>("g8r_file").unwrap());
