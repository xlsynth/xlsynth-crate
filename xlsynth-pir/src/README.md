...
- Every parsed function begins with a sentinel node at index `0` whose payload
  is `Nil`.
- `NodePayload::Param` identifies parameter nodes. Each node owns its name,
  type, and textual `id=`; there is no separate parameter record or ID type.
- `Fn.params: Vec<NodeRef>` specifies signature order. The parser and builder
  initially place parameters after the sentinel, but consumers must follow the
  references: signature order is independent of node storage order and textual
  IDs. `Fn::param_nodes()` borrows nodes in signature order.
- Parameter nodes have no operands and act as leaves in dependency walks.
  Computation nodes refer to them using ordinary `NodeRef` operands.
- Reordering or compacting a graph must remap `Fn.params` along with operands
  and the return reference. `compact_and_toposort_in_place` does this together.
...
- When a function wants to return a parameter directly—without adding an
  ...
  its entire body. Parsing reuses the signature's parameter node: the return
  reference and signature reference point to the same node.
- The `returning_parameter_uses_explicit_param_node` test in `ir.rs`
  demonstrates this round-trip with no duplicate parameter allocation.
