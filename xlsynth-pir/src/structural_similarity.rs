...
fn compute_node_depth(f: &Fn, node_ref: NodeRef, child_depths: &[usize]) -> usize {
    match &f.get_node(node_ref).payload {
        ...
        | NodePayload::ExtClz { .. }
        | NodePayload::ExtNormalizeLeft { .. } => 0,
...
pub fn collect_structural_entries(f: &Fn) -> (Vec<StructuralEntry<FwdHash>>, Vec<usize>) {
    ...
    for node_ref in order {
        ...
        match &node.payload {
            ...
            | NodePayload::ExtNormalizeLeft { arg: tuple, .. }
            ...
            NodePayload::ExtCarryOut { lhs, rhs, c_in } => {
                child_hashes.push(hashes[lhs.index]);
                child_depths.push(depths[lhs.index]);
                child_hashes.push(hashes[rhs.index]);
                child_depths.push(depths[rhs.index]);
