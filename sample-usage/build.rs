...
fn make_readme_snippet_file() {
    ...
    // We'll write a new .rs file into OUT_DIR that becomes part of our test
    // module. We'll wrap the snippet in something that makes it testable.
    // For example: we define a `mod readme_snippet { ... }` with #[test]
    // functions.
    //
    // If your snippet has `fn main() {}`, we can call that from the test
    // function, so that `cargo test` effectively runs what your snippet
    // does.
    let test_code = format!(
        r#"// DO NOT EDIT THIS FILE. It is generated by build.rs from README.md.
mod readme_snippet {{
    {code}
