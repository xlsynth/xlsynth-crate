...
## Generation and input format

The mixed semantic campaign compares native generated RTL in iverilog with
PIR evaluation. It mixes scalar/aggregate combinational blocks with
...
in the mixed campaign and the focused targets using its options. They remain enabled
for iverilog and Yosys mapping/formal checks. Other
operations retain their wider limits except for the multiply operand cap below.
This bounds divider synthesis cost and avoids iverilog multiword-division
...
`max_div_mod_bit_width` unset by default. The mixed campaign caps signed/unsigned
...
are independent within those limits. These targets select the proven-safe
...
The mixed campaign includes all six public `ext_*` operations, zero-bit values,
zero-bit ports/registers, and aggregate gating. Calls, loops, partial products,
assertions, covers, traces, and event tokens are excluded.
The dedicated Yosys targets disable extensions and zero-width generation
and use smaller scalar graphs to keep synthesis and proofs bounded. Existing hierarchy/extern targets remain separate
structural tests; the existing focused partial-product target is unchanged.

Version 1 mixed-campaign inputs have a 48-byte prefix: `XBCF`, version byte `1`,
...
exclusively for final connectivity. For the mixed campaign the header is
...
The mixed target prints cumulative `codegen-coverage` JSON on the first
...
cases**; they are not proof of semantic coverage. Tool resource failures are
recorded separately as inconclusive. Checking uses 16 input vectors for
...
`cargo run --release --example random_eval -- --seed 1 --samples 1000 --artifact-dir /path/to/reproducer` generates graphs using
...
guided target, with the same tool checks, stimuli and width caps.
...
semantic check. No extra iverilog feature is needed in this standalone fuzz crate.
iverilog compiles
...
Yosys/ABC mapping has a 120-second timeout. Tool timeouts and resource exhaustion
are inconclusive checks, and campaigns continue with the next sample. Other
tool failures and semantic mismatches are fatal. Timed-out child process groups
are terminated. Set libFuzzer's `-rss_limit_mb`
...
cargo run --release --example coverage -- \
  --samples 1000 --seed 1 --bytes 2048 \
...
cargo run --release --example coverage -- \
  --corpus corpus/fuzz_codegen_native_semantics --check \
  > native-coverage.json
```

`--corpus` accepts a directory tree or a single reproducer file.
...
cargo fuzz run --sanitizer none --features external-yosys \
  fuzz_codegen_yosys_combo -- -runs=1
```
...
retain narrower scalar complexity limits to bound mapping cost. These paths
...
These are functional checks; they do not model cell timing or SDF delays.
...
```sh
cargo run --release --example coverage -- \
  --samples 200 --seed 1 --bytes 2048 \
  --write-corpus corpus/fuzz_codegen_native_semantics
cargo fuzz run --sanitizer none fuzz_codegen_native_semantics -- \
  -max_total_time=10800 -timeout=180 -rss_limit_mb=4096 -jobs=4 -workers=4
```

Select `--features external-yosys` for the dedicated mapping/formal targets. Existing corpus contents are reused by
libFuzzer. Save a final generation census or explicitly check every input with:

```sh
cargo run --release --example coverage -- \
  --corpus corpus/fuzz_codegen_native_semantics
cargo run --release --example coverage -- \
  --corpus corpus/fuzz_codegen_native_semantics --check
```
...
artifact. iverilog tools are checked for native simulation.
Mapping preflight loads the configured
...
The reusable configuration, Liberty loading, mapping scripts, and probe live in
`xlsynth-g8r::netlist::yosys`; the fuzz harness only caches the result and
decides how to report startup or per-sample failures. Verilog and SystemVerilog
use the same mapping passes with an explicitly selected frontend.
...
iverilog compilation and finite self-checking testbench execution use
`xlsynth-test-helpers::iverilog::IcarusToolchain`. Persistent vector/cycle
simulation uses `xlsynth-test-helpers::rtl_sim`; block interfaces and reference
expectations remain in this fuzz crate.

libFuzzer's RSS limit measures the fuzzer process, not Yosys/ABC/iverilog children.
...
iverilog simulation. Failures expose incorrect arithmetic,
signedness, comparisons, shifts, selection, gating, or port connectivity.

## `fuzz_codegen_native_semantics`

Uses mixed block generation to compare iverilog simulation of generated
...
module selection, and independent instance connectivity. A required iverilog
...
iverilog simulation of a self-checking testbench verifies external-instance
