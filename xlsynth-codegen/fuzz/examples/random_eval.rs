...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" => {
                println!(
                    "random_eval [--seed N | --case-seed N] [--samples N] [--duration SECONDS] [--artifact-dir PATH] [--no-check]"
                );
                return Ok(());
            }
    ...
    if check {
        xlsynth_codegen_fuzz::preflight::validate(
            xlsynth_codegen_fuzz::preflight::Oracles::Icarus,
        )?;
    }
    ...
    for index in 0..samples {
        ...
        if let Some(dir) = &directory {
            ...
            // Save before graph generation so even a generator panic is
            // replayable.
            let progress = serde_json::json!({"generator_version":GENERATOR_VERSION,
                "worker_seed":seed, "case_seed":this_seed,
        ...
        let case = FuzzCase::random(this_seed);
        ...
        {
            println!("codegen-coverage {}", report.json());
