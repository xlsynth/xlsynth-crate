...
def test_release_without_publish_order_is_ineligible_regardless_of_version(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    assert gen_version_compat._get_release_ineligibility_reason(
        "v0.48.0", "0.48.0"
    ) == ("v0.48.0 does not contain publish_order.toml")
...
def test_get_version_mapping_reuses_cached_row_without_eligibility_check(
    ...
    def get_reason(tag: str, crate_version: str):
