...
#[repr(C)]
/// Per-invocation events surfaced by the supported standalone callbacks.
struct StandaloneEvents {
    assert_messages: Vec<String>,
}

#[repr(C)]
/// Callback table passed to LLVM-generated standalone AOT entrypoints.
struct StandaloneInstanceContext {
    vtable: [*const (); 12],
}

/// Aborts if the generated artifact reaches a callback outside the supported
/// ABI.
unsafe extern "C" fn unsupported_callback() {
    std::process::abort();
}

/// Records one runtime assertion message emitted by the generated entrypoint.
unsafe extern "C" fn record_assertion(
    _context: *mut StandaloneInstanceContext,
    message: *const std::ffi::c_char,
    events: *mut StandaloneEvents,
) {
    if !events.is_null() && !message.is_null() {
        let message = unsafe { std::ffi::CStr::from_ptr(message) }
            .to_string_lossy()
            .into_owned();
        unsafe { &mut *events }.assert_messages.push(message);
    }
}

/// Allocates scratch memory requested by the generated standalone entrypoint.
...
/// Frees scratch memory requested by the generated standalone entrypoint.
...
/// Linked function signature exported by a compiled standalone AOT object.
type StandaloneEntrypoint = unsafe extern "C" fn(
    *const *const u8,
    *const *mut u8,
    *mut std::ffi::c_void,
    *mut StandaloneEvents,
    *mut StandaloneInstanceContext,
    *mut std::ffi::c_void,
    i64,
) -> i64;
...
pub struct StandaloneRunner {
    ...
    instance_context: StandaloneInstanceContext,
}
...
impl StandaloneRunner {
    ...
    pub unsafe fn new(
        ...
        let output_ptrs = output_buffers
            ...
            .collect::<Vec<_>>();
        let vtable = [
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            record_assertion as *const (),
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            unsupported_callback as *const (),
            allocate_buffer as *const (),
            deallocate_buffer as *const (),
            unsupported_callback as *const (),
        ];
        Ok(Self {
            entrypoint: unsafe {
                std::mem::transmute::<*const (), StandaloneEntrypoint>(entrypoint)
            },
            input_buffers,
            output_buffers,
            input_ptrs,
            output_ptrs,
            temp_buffer: AlignedBuffer::new(layout.temp_buffer_size, layout.temp_buffer_alignment)?,
            events: StandaloneEvents {
                assert_messages: Vec::new(),
            },
            instance_context: StandaloneInstanceContext { vtable },
        })
    }
    ...
    pub fn run_raw(&mut self) {
        self.events.assert_messages.clear();
        unsafe {
            (self.entrypoint)(
